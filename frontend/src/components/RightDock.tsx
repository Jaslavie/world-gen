...
function Section({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="space-y-3">
      <h2 className="px-1 text-[10px] font-semibold uppercase tracking-widest text-faint">
        {title}
      </h2>
      {children}
    </div>
...
function PromptBlock() {
  ...
  const words = Array.from(
    new Set(
      (world?.entities ?? []).map((e) => e.type).filter((t) => t !== "agent"),
    ),
  );
...
function MappedAssets() {
  ...
  return (
    <Panel
      title="Objects"
      tag={mappedAssets.length ? `${mappedAssets.length} objects` : undefined}
    >
      {mappedAssets.length === 0 ? (
        <p className="text-[13px] text-faint">
          objects appear here once compiled
        </p>
      ) : (
              ...
              >
                {a.png ? (
                  <img
                    src={a.png}
                    alt={a.label}
                    className="pixelated mx-auto h-10 w-10 object-contain"
                  />
                ) : (
  ...
  return (
    <Panel title="Goal status" tag="agent progress">
      <div className="mb-1 flex items-end justify-between">
        <span
          className={`text-2xl font-semibold tabular-nums ${success ? "text-ok" : "text-ink"}`}
        >
          {goalPct}%
        </span>
        {success && (
          <span className="text-[12px] font-semibold text-ok">complete</span>
        )}
      </div>
...
function ToolCalls() {
  ...
  return (
    <Panel
      title="Tool calls"
      tag={toolCalls.length ? `${toolCalls.length} total` : undefined}
    >
      {toolCalls.length === 0 ? (
        ...
        <>
          <div className="max-h-44 space-y-1 overflow-y-auto pr-1">
            {shown.map((c) => (
              <div
                key={c.n}
                className="flex items-center gap-2 rounded-md bg-canvas px-2 py-1 text-[12px]"
              >
                <span className="font-mono font-medium text-accent">
                  {c.tool}
                </span>
                {c.arg && (
                  <span className="font-mono text-ink/70">{c.arg}</span>
                )}
              </div>
            ))}
          ...
          {toolCalls.length > 6 && (
            <button
              ...
              className="mt-2 text-[11px] text-faint hover:text-muted"
            >
              {showAll
                ? "collapse"
                : `view full audit log (${toolCalls.length})`}
            </button>
          )}
        </>
      )}
