"""
Deterministic action primitives the agent calls to interact with the world.
Action calls will fail if the precondition is not met so agent can never reach an illegal state.
...
class SpatialTools:
    ...
    ACTIONS = ("move", "pick", "place", "toggle", "wait")
    ...
    def move(self, snap: Snapshot, direction: str, agent: str = "agent") -> Snapshot:
        ...
        # If fails, return the snapshot
        if (not self.v.in_bounds(snap, dst) or dst in snap["walls"]
                or dst in self.v.blocked_cells(snap, ignore=(agent,))):
            print(f"Invalid move: {direction} is not a valid direction")
            return snap
        
        # If valid, update the position of the agent
        snap["pos"][agent] = dst

        # If the agent is carrying an item, update the position of the item
    ...
    def observe(self, snap: Snapshot, agent: str = "agent") -> dict[str, Any]:
        ...
        return {
            ...
            "objective": self.v.describe(snap["objective"]),
            "clauses": [{"clause": c, "satisfied": ok}
                        for c, ok in self.v.clause_status(snap["objective"], snap)],
            "success": self.v.get_success(snap),
            "entities": entities,
        }
